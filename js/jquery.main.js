...
jQuery(function(){
	...
	jQuery('.search').click(function (e) {
        ...
        if (jQuery('.mnavlist').is(':visible')) {
			jQuery('.mnavlist').slideToggle(350);
		}
	...
	// Resource page highlight selected options
	if(jQuery("#views-exposed-form-resources-page").length > 0) {

		var industry = jQuery("#edit-field-industry-tid option:selected").index();
		var solution = jQuery("#edit-field-solution-tid option:selected").index();

		if(industry != 0) {
			//jQuery('#edit-field-industry-tid>option:selected').addClass("highlight");	
			jQuery('select#edit-field-industry-tid').addClass("highlight");
		}
		if(solution != 0) {
			jQuery('select#edit-field-solution-tid').addClass("highlight");
		}
	}

	// Case study open close.
	jQuery('.column-wrapper .columns .case-nav').each(function (i) {
		var items = jQuery(this).find('> li');
		if (items.length > 2) {
			var height = items.eq(0).outerHeight() + items.eq(1).outerHeight();
			jQuery(this).css({'height':height+'px'});
			jQuery(this).data('closedHeight', height);
		}
	});
	jQuery('.column-wrapper .columns .more').click(function (e) {
		e.preventDefault();
		if (jQuery(this).hasClass('open')) {
			var wrap = jQuery(this).prev();
			var height = wrap.data('closedHeight');
	...
	// Open Mobile Nav
	jQuery('a#mnav').bind('click', function(){
		jQuery(this).siblings('.mnavlist').slideToggle(350);

		if (jQuery('#block-search-form').is(':visible')) {
			jQuery('#block-search-form').slideToggle(350);
		}
	});

	// Open Mobile Nav Tabs
	jQuery('.mnavlist ul li a.parent').bind('click', function(){
		jQuery(this).siblings('ul').slideToggle(350);
	});

	// Set up tabs
	var startAt = 0;
	if (location.hash) {
		var staff = jQuery('.tab-panels').find(location.hash);
		var panel = staff.closest('.panel');
		startAt = panel.index();
	}
  	jQuery(".tabs").tabby({
		startAt:startAt
...
*/
	...
	jQuery('#newsBox img').hover(function(){
		jQuery('#newsBox img').filter(':animated').stop(true).css({ 'width': '158px', 'height': '105px', 'z-index': '0' });
	...
	jQuery('#edit-name').attr('placeholder', 'YOUR NAME')
	jQuery('#edit-comment-body-und-0-value').val('COMMENT').css('color', '#949494');
	jQuery('#edit-comment-body-und-0-value').focus(function () {
		if (jQuery(this).val() === 'COMMENT') {
			jQuery(this).val('').css('color', 'inherit');
		}
	});
	jQuery('#edit-comment-body-und-0-value').blur(function () {
		if (jQuery(this).val().length == 0) {
			jQuery(this).val('COMMENT').css('color', '#949494');
		}
	});
});
...
jQuery(window).on('load', function(){
	...
	if(window.location.hash.indexOf('-anchor') > 0) {
		var anchorTarget = window.location.hash.replace('-anchor', '');
		jQuery(anchorTarget).parents('.field-faqs').trigger('click');
		setTimeout(function() {
			var anchorOffset = jQuery(anchorTarget).offset().top - 80;
	...
	jQuery(".block.start-open h3").trigger('click');
});
...
function initOpenClose() {
	...
	jQuery('.sidebar-accordions').openClose({
		activeClass: 'active',
		opener: 'h3',
		slider: '.view',
		animSpeed: 400,
